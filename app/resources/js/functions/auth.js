...
class ChangeProfileImage extends Base {
  ...
  arrows() {
    ...
    this.urlImage = async (e) => {
      ...
      this.img && (await fetchEndpoint(`/images/${this.img}`, 'DELETE'))
      if ($('span.imgdata')) $('span.imgdata').dataset.img = ''
      this.img = null
    }
  }

  events() {
    this.uploadform.image.addEventListener('change', this.uploadImage, {
      once: true,
    })
    this.upload.addEventListener('click', () => this.uploadform.image.click())
...
class ChangePassword extends Base {
  ...
  handleData(data) {
    ...
    if (data.user) location.assign('/dashboard')
  }
}
